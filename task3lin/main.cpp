...
//void merge(double *a, double *b1, int _k1, int _k2, int _k3) {
//    int k1=_k1, k2=_k2, k=k1, kk=k2;
//
...
//    for(int i = _k1;i<_k3+1;i++) a[i]=b1[i];
//}
...
void localSort(double* a, int N, int threadNum=1){
    omp_set_dynamic(0); 
...
void localParSort(double* a, int N, int threadNum=1){
    omp_set_dynamic(0); 
    omp_set_num_threads(threadNum);
#pragma omp parallel
    {
